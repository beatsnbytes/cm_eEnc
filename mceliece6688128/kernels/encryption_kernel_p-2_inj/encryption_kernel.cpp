...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+12))/(ALIGNED_BYTES*2)); i++){
		#pragma HLS PIPELINE II=1
...
void encryption_kernel(unsigned char *s_1, unsigned char *s_2, unsigned char *e, aligned_width *pk_1, aligned_width *pk_2, v_vector *v, key_vector *key)
{
    ...
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=4160 bundle=gmem3
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=4160 bundle=gmem4
	...
	static hls::stream<custom_width> pk_stream_2("pk stream 1");
	#pragma HLS STREAM variable=pk_stream_2 depth=2
	static hls::stream<custom_width> pk_stream_1("pk stream 2");
	#pragma HLS STREAM variable=pk_stream_2 depth=2
