...
#define BIT_WIDTH 1024// 157 1B values
...
//    unsigned char e_2_mat[SYS_N/8];
	#pragma HLS ARRAY_PARTITION variable=e_2_mat cyclic factor=157//109
...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+12))/(ALIGNED_BYTES*4)); i++){
		#pragma HLS PIPELINE II=1
...
//TODO try with 64 bits and see how to keep the same hls optimizations
	...
	#pragma HLS PIPELINE II=1
		...
		b ^= pk_tmp.range(927, 0) & e_tmp.range(927, 0);
...
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=57  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=s_2     offset=slave depth=57  bundle=gmem1
	#pragma HLS INTERFACE m_axi     port=s_3     offset=slave depth=57  bundle=gmem2
	#pragma HLS INTERFACE m_axi     port=s_4     offset=slave depth=57  bundle=gmem3
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=836  bundle=gmem4
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=2080 bundle=gmem1
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=2080 bundle=gmem2
	#pragma HLS INTERFACE m_axi     port=pk_3    offset=slave depth=2080 bundle=gmem3
	#pragma HLS INTERFACE m_axi     port=pk_4    offset=slave depth=2080 bundle=gmem4
