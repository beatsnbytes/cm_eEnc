...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	int i;
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+12))/(ALIGNED_BYTES)); i++){
		#pragma HLS PIPELINE II=1
...
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=208  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=836  bundle=gmem2
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=8320 bundle=gmem3 //TODO fix the depth here
