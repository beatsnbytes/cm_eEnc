    ...
    #ifdef ENCRYPTION_KERNEL
	{
		...
		#ifdef ALVEO
        {
    		...
    		for(i=0; i<PARALLELIZATION_FACTOR; i++){
    			posix_memalign((unsigned char **)&ptr_pk_in[i], 4096, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+92))/PARALLELIZATION_FACTOR);
    	    ...
    	    for(i=0; i<PARALLELIZATION_FACTOR; i++){
    	        //PK
    	        buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+92))/PARALLELIZATION_FACTOR, ptr_pk_in[i], NULL);
    	        //S
        ...
        #ifdef ZCU
        {
            ...
            buffer_v_in = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*16, NULL, NULL);
            buffer_key_in = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*32, NULL, NULL);

            for(i=0; i<PARALLELIZATION_FACTOR; i++){
                //PK
                buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+92))/PARALLELIZATION_FACTOR, NULL, NULL);
                //S
                buffer_s_out[i] = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(unsigned char)*(SYND_BYTES)/PARALLELIZATION_FACTOR, NULL, NULL);
            }

            
            buffer_e_out = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(unsigned char)*(SYS_N/8), NULL, NULL);

            ptr_v_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_v_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*16, 0, NULL, NULL, NULL);
            ptr_key_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_key_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*32, 0, NULL, NULL, NULL);

            for(i=0; i<PARALLELIZATION_FACTOR; i++){
                //PK
                ptr_pk_in[i] = (unsigned char *) clEnqueueMapBuffer(commands, buffer_pk_in[i], true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+92))/PARALLELIZATION_FACTOR, 0, NULL, NULL, NULL);

                //S_BUFF
                ptr_s_out[i] = (unsigned char *) clEnqueueMapBuffer(commands, buffer_s_out[i], true, CL_MAP_READ, 0, sizeof(unsigned char)*(SYND_BYTES)/PARALLELIZATION_FACTOR, 0, NULL, NULL, NULL);
            }

            ptr_e_out = (unsigned char *) clEnqueueMapBuffer(commands, buffer_e_out, true, CL_MAP_READ, 0, sizeof(unsigned char)*(SYS_N/8), 0, NULL, NULL, NULL);
