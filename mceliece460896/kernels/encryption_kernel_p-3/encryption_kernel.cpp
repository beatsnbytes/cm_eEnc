...
void dump_to_e_1_stream(hls::stream<unsigned char> &e_1_stream, hls::stream<unsigned char> &e_1_internal_stream){
	...
	for(i=0; i<(SYND_BYTES)/3; i++){
		e_1_stream.write(e_1_internal_stream.read());
	}
}

void dump_to_e_2_stream(hls::stream<custom_width> &e_2_stream_1, hls::stream<custom_width> &e_2_stream_2, hls::stream<custom_width> &e_2_stream_3, unsigned char *e_2_mat){
...
void compute_e_streams(hls::stream<unsigned char> &e_1_internal_stream_1, hls::stream<unsigned char> &e_1_internal_stream_2, hls::stream<unsigned char> &e_1_internal_stream_3, unsigned char *e_2_mat, hls::stream<unsigned char> &e_out_internal_stream, hls::stream<gf_value> &indexes_stream){
	...
	#pragma HLS PIPELINE
		...
		}else if((i>=(SYND_BYTES)/3) && (i<2*(SYND_BYTES)/3)){
			e_1_internal_stream_2.write(e_int);
		}else if((i>=2*(SYND_BYTES)/3) && (i<SYND_BYTES)){
...
static void gen_e(hls::stream<unsigned char> &e_1_stream_1, hls::stream<unsigned char> &e_1_stream_2, hls::stream<unsigned char> &e_1_stream_3, hls::stream<custom_width> &e_2_stream_1, hls::stream<custom_width> &e_2_stream_2, hls::stream<custom_width> &e_2_stream_3, hls::stream<unsigned char> &e_out_stream,  v_vector *v, key_vector *key)
{
	int i, j, eq, count;
...
#pragma HLS dataflow
	...
	compute_e_streams(e_1_internal_stream_1, e_1_internal_stream_2, e_1_internal_stream_3, e_2_mat, e_out_internal_stream, indexes_stream);
	...
	dump_to_e_2_stream(e_2_stream_1, e_2_stream_2, e_2_stream_3, e_2_mat);
...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_BYTES*3)); i++){
		#pragma HLS PIPELINE II=1
...
void pk_stream_resizer(hls::stream<custom_width> &pk_stream, hls::stream<aligned_width> &pk_aligned_stream){
	...
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_VALUES*3)); i++){
	#pragma HLS PIPELINE II=1
		if(remnant_bits<BIT_WIDTH){
			...
			//write to the output stream one 105 bundle
...
void encryption_kernel(unsigned char *s_1, unsigned char *s_2, unsigned char *s_3, unsigned char *e, aligned_width *pk_1, aligned_width *pk_2, aligned_width *pk_3, v_vector *v, key_vector *key)
{
	...
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=576  bundle=gmem3
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=1365 bundle=gmem4
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=1365 bundle=gmem5
	#pragma HLS INTERFACE m_axi     port=pk_3    offset=slave depth=1365 bundle=gmem6
    #pragma HLS INTERFACE m_axi     port=v       offset=slave depth=1 bundle=gmem7
	#pragma HLS INTERFACE m_axi     port=key     offset=slave depth=1 bundle=gmem8
	...
	gen_e(e_1_stream_1, e_1_stream_2, e_1_stream_3, e_2_stream_1, e_2_stream_2, e_2_stream_3, e_out_stream, v, key);
