...
void dump_to_e_2_stream(hls::stream<custom_width> &e_2_stream_1, unsigned char *e_2_mat){
...
void compute_e_streams(hls::stream<unsigned char> &e_1_internal_stream_1, unsigned char *e_2_mat, hls::stream<unsigned char> &e_out_internal_stream, hls::stream<gf_value> &indexes_stream){
...
static void gen_e(hls::stream<unsigned char> &e_1_stream_1, hls::stream<custom_width> &e_2_stream_1, hls::stream<unsigned char> &e_out_stream,  v_vector *v, key_vector *key)
{
	int i, j, eq, count;
...
#pragma HLS dataflow
	...
	compute_e_streams(e_1_internal_stream_1, e_2_mat, e_out_internal_stream, indexes_stream);

	dump_to_e_1_stream(e_1_stream_1, e_1_internal_stream_1);

	dump_to_e_2_stream(e_2_stream_1, e_2_mat);
...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_BYTES)); i++){
		#pragma HLS PIPELINE II=1
...
void pk_stream_resizer(hls::stream<custom_width> &pk_stream, hls::stream<aligned_width> &pk_aligned_stream){
	...
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_VALUES)); i++){
	#pragma HLS PIPELINE II=1
...
void encryption_kernel(unsigned char *s_1, unsigned char *e, aligned_width *pk_1, v_vector *v, key_vector *key)
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=156   bundle=gmem
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=576  bundle=gmem1
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=4095 bundle=gmem2
	...
	gen_e(e_1_stream_1, e_2_stream_1, e_out_stream, v, key);
