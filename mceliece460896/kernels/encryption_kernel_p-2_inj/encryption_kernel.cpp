...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+92))/(ALIGNED_BYTES*2)); i++){
		#pragma HLS PIPELINE II=1
...
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=78  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=s_2     offset=slave depth=78  bundle=gmem1
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=576  bundle=gmem2
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=2496 bundle=gmem3
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=2496 bundle=gmem4
