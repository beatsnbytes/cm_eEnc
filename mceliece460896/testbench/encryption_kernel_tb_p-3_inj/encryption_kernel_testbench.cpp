...
int main () { 
    ...
    //Inputs
    aligned_width *pk_reference_input_1 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+92))/(ALIGNED_BYTES*3)));
    aligned_width *pk_reference_input_2 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+92))/(ALIGNED_BYTES*3)));
    aligned_width *pk_reference_input_3 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+92))/(ALIGNED_BYTES*3)));
    ...
    unsigned char *s_kernel_output_1 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/3);
    unsigned char *s_kernel_output_2 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/3);
    unsigned char *s_kernel_output_3 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/3);
...
//		printf("key = %d\n", key.to_uint());
    ...
    encryption_kernel_active(s_kernel_output_1, s_kernel_output_2, s_kernel_output_3, e_kernel_output, pk_reference_input_1, pk_reference_input_2, pk_reference_input_3, v_reference_input, key_reference_input);
    ...
    for(i=0; i<(SYND_BYTES)/3; i++){
    	if(*(s_kernel_output_2 + i)!= *(s_golden_output + i + (SYND_BYTES)/3)){
    		printf("ERROR! Expected s_2[%d]=%d but got %d\n", i, *(s_golden_output + i + (SYND_BYTES)/3), *(s_kernel_output_2 + i));
    		ret=1;
    	}
    }

    for(i=0; i<(SYND_BYTES)/3; i++){
    	if(*(s_kernel_output_3+ i)!= *(s_golden_output + i + 2*(SYND_BYTES)/3)){
    		printf("ERROR! Expected s_2[%d]=%d but got %d\n", i, *(s_golden_output + i + 2*(SYND_BYTES)/3), *(s_kernel_output_3 + i));
    		ret=1;
    	}
    }
