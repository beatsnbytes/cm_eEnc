...
)
{
	...
	#ifdef ENCRYPTION_KERNEL
	{
		...
		// 		memcpy( ptr_pk_in[0]+j*(PK_ROW_BYTES+44), pk + (j*PK_ROW_BYTES), sizeof(unsigned char)*(PK_ROW_BYTES));
		// 	}
		// }
...
)
{
	...
	#ifdef DECRYPTION_KERNEL
	{
		...
		memcpy(ptr_sk_irr_in, sk+40, sizeof(unsigned char)*128);
		memcpy(ptr_sk_fwd_in, sk+40+IRR_BYTES, sizeof(unsigned char)*(23*256));

		for(i=0; i<23; i++){
			memcpy(ptr_sk_rev_in+i * 256, sk+40+IRR_BYTES+(22-i)*256, sizeof(unsigned char)*256);
		}
		memcpy(ptr_c_in, c, sizeof(unsigned char)*(SYND_BYTES));

		clEnqueueMigrateMemObjects(commands, 4, buffer_list_dec_in, 0, 0, NULL, &event_migr_tokern);
		clEnqueueTask(commands, decryption_kernel, 1, &event_migr_tokern, &event_enq);
		clEnqueueMigrateMemObjects(commands, 2, buffer_list_dec_out, CL_MIGRATE_MEM_OBJECT_HOST, 1, &event_enq, &event_migr_tohost);
		clWaitForEvents(1, &event_migr_tohost);

		cl_profile_print(&event_enq, 1, sum_list_event_enq, &times_event_enq);
		memcpy(e, ptr_e_out_dec, sizeof(unsigned char)*(SYS_N/8));
		ret_decrypt=*ptr_check_out;
