...
//DECRYPTION//
cl_mem buffer_sk_irr_in;
cl_mem buffer_sk_rev_in;
cl_mem buffer_sk_fwd_in;
cl_mem buffer_c_in;
cl_mem buffer_check_out;
cl_mem buffer_e_out_dec;

cl_mem buffer_list_dec_in[4];
cl_mem buffer_list_dec_out[2];

unsigned char *ptr_sk_irr_in;
unsigned char *ptr_sk_rev_in;
unsigned char *ptr_sk_fwd_in;

unsigned char *ptr_c_in;
int *ptr_check_out;
unsigned char *ptr_e_out_dec;

cl_kernel decryption_kernel;
cl_context context;
    ...
    #ifdef ENCRYPTION_KERNEL
	{
		...
		#ifdef ALVEO
        {
    		...
    		for(i=0; i<PARALLELIZATION_FACTOR; i++){
    			// posix_memalign((unsigned char **)&ptr_pk_in[i], 4096, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+44))/PARALLELIZATION_FACTOR);
                posix_memalign((unsigned char **)&ptr_pk_in[i], 4096, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES))/PARALLELIZATION_FACTOR);
    	    ...
    	    for(i=0; i<PARALLELIZATION_FACTOR; i++){
    	        //PK
    	        buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES))/PARALLELIZATION_FACTOR, ptr_pk_in[i], NULL);
                // buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+44))/PARALLELIZATION_FACTOR, ptr_pk_in[i], NULL);
    	        //S
		...
		#ifdef ZCU
        {
			...
			for(i=0; i<PARALLELIZATION_FACTOR; i++){
				//PK
				// buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+44))/PARALLELIZATION_FACTOR, NULL, &err);
                buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES))/PARALLELIZATION_FACTOR, NULL, &err);
				//S
			...
			for(i=0; i<PARALLELIZATION_FACTOR; i++){
				//PK
				// ptr_pk_in[i] = (unsigned char *) clEnqueueMapBuffer(commands, buffer_pk_in[i], true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+44))/PARALLELIZATION_FACTOR, 0, NULL, NULL, &err);
                ptr_pk_in[i] = (unsigned char *) clEnqueueMapBuffer(commands, buffer_pk_in[i], true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES))/PARALLELIZATION_FACTOR, 0, NULL, NULL, &err);
				//S_BUFF
        ...
        buffer_list_enc_out[PARALLELIZATION_FACTOR] = buffer_e_out;
                                                            ...
                                                            //DECRYPTION//
    #ifdef DECRYPTION_KERNEL
    {             

        const char *kernel_name = "decryption_kernel";
        decryption_kernel = clCreateKernel(program, kernel_name, NULL);

		#ifdef ALVEO
        {
			posix_memalign((unsigned char **)&ptr_sk_fwd_in, 4096, sizeof(unsigned char)*(23*256));
			posix_memalign((unsigned char **)&ptr_sk_rev_in, 4096, sizeof(unsigned char)*(23*256));
			posix_memalign((unsigned char **)&ptr_sk_irr_in, 4096, sizeof(unsigned char)*(128));

			posix_memalign((unsigned char **)&ptr_c_in, 4096, sizeof(unsigned char)*(SYND_BYTES));
			posix_memalign((unsigned char **)&ptr_check_out, 4096, sizeof(int));
			posix_memalign((unsigned char **)&ptr_e_out_dec, 4096, sizeof(unsigned char)*(SYS_N/8));


			buffer_sk_fwd_in = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(23*256), ptr_sk_fwd_in, &err);
			buffer_sk_rev_in = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(23*256), ptr_sk_rev_in, &err);
			buffer_sk_irr_in = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(128), ptr_sk_irr_in, &err);

			buffer_c_in = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(SYND_BYTES), ptr_c_in, &err);
			buffer_check_out = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(int), ptr_check_out, &err);
			buffer_e_out_dec = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(SYS_N/8), ptr_e_out_dec, &err);
        }
        #endif
		...
		#ifdef ZCU
        {
			...
			buffer_sk_fwd_in = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY, sizeof(unsigned char)*(23*256), NULL, &err);
			buffer_sk_rev_in = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY, sizeof(unsigned char)*(23*256), NULL, &err);
			buffer_sk_irr_in = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY, sizeof(unsigned char)*(128), NULL, &err);

			buffer_c_in = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_READ_ONLY, sizeof(unsigned char)*(SYND_BYTES), NULL, &err);
			buffer_check_out = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_WRITE_ONLY, sizeof(int), NULL, &err);
			buffer_e_out_dec = clCreateBuffer(context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_WRITE_ONLY, sizeof(unsigned char)*(SYS_N/8), NULL, &err);

			ptr_sk_rev_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_sk_rev_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(23*256), 0, NULL, NULL, &err);
			ptr_sk_fwd_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_sk_fwd_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(23*256), 0, NULL, NULL, &err);
			ptr_sk_irr_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_sk_irr_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(128), 0, NULL, NULL, &err);

			ptr_c_in = (unsigned char *) clEnqueueMapBuffer(commands, buffer_c_in, true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(SYND_BYTES), 0, NULL, NULL, &err);
			ptr_check_out = (int *) clEnqueueMapBuffer(commands, buffer_check_out, true, CL_MAP_READ, 0, sizeof(int), 0, NULL, NULL, &err);
			ptr_e_out_dec = (unsigned char *) clEnqueueMapBuffer(commands, buffer_e_out_dec, true, CL_MAP_READ, 0, sizeof(unsigned char)*(SYS_N/8), 0, NULL, NULL, &err);

        }
		#endif

        clSetKernelArg(decryption_kernel, 0, sizeof(cl_mem), &buffer_check_out);
        clSetKernelArg(decryption_kernel, 1, sizeof(cl_mem), &buffer_e_out_dec);
        clSetKernelArg(decryption_kernel, 2, sizeof(cl_mem), &buffer_sk_irr_in);
        clSetKernelArg(decryption_kernel, 3, sizeof(cl_mem), &buffer_sk_rev_in);
        clSetKernelArg(decryption_kernel, 4, sizeof(cl_mem), &buffer_sk_fwd_in);
        clSetKernelArg(decryption_kernel, 5, sizeof(cl_mem), &buffer_c_in);

        buffer_list_dec_in[0] = buffer_sk_irr_in;
        buffer_list_dec_in[1] = buffer_sk_rev_in;
        buffer_list_dec_in[2] = buffer_sk_fwd_in;
        buffer_list_dec_in[3] = buffer_c_in;

        buffer_list_dec_out[0] = buffer_check_out;
        buffer_list_dec_out[1] = buffer_e_out_dec;
    }
    #endif
