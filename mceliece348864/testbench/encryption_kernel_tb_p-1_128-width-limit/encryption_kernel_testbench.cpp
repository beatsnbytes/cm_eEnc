...
int main () { 
    ...
    //Inputs
    aligned_width *pk_reference_input_1 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_BYTES)));

    ap_uint<128> *key_reference_input = (ap_uint<128> *) malloc(sizeof(ap_uint<128>));
    ap_uint<128> *v_reference_input = (ap_uint<128> *) malloc(sizeof(ap_uint<128>)*2);
...
//    *key_reference_input=0;
	for(i=0; i<2; i++){
    	(key_reference_input[i]) = 0;
		for(j=0; j<16; j++){
			key = 0;
			fscanf(key_fptr, "%d\n", &key);
			(key_reference_input[i]) |= key<<(j*8);
		}
    ...
    encryption_kernel_active(s_kernel_output_1, e_kernel_output, pk_reference_input_1, v_reference_input, key_reference_input);
