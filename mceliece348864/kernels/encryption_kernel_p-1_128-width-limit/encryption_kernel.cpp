...
#define ALIGNED_VALUES 10//85
#define BIT_WIDTH 80//680
#define ALIGNED_WIDTH 128 //1024
#define ALIGNED_BYTES 16//128
...
void load_v_key(v_vector *v_local_vector, key_vector *key_local_vector, v_vector *v, v_vector *key){

	*v_local_vector = *v;

//	*key_local_vector = *key;
	key_local_vector->range(127, 0) = *(key+0);
	key_local_vector->range(255, 128) = *(key+1);
...
void compute_indexes_mat(hls::stream<gf_value> &indexes_stream, v_vector *v, v_vector *key){
...
static void gen_e(hls::stream<unsigned char> &e_1_stream_1, hls::stream<custom_width> &e_2_stream_1, hls::stream<unsigned char> &e_out_stream,  v_vector *v, v_vector *key)
{
	int i, j, eq, count;
...
void encryption_kernel(unsigned char *s_1, unsigned char *e, aligned_width *pk_1, v_vector *v, v_vector *key)
{
    ...
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=16320  bundle=gmem2
    #pragma HLS INTERFACE m_axi     port=v       offset=slave depth=1  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=key     offset=slave depth=2  bundle=gmem
