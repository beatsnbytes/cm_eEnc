...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+44))/(ALIGNED_BYTES*4)); i++){
		#pragma HLS PIPELINE II=1
...
void compute_b_mat(hls::stream<b_width> &b_stream, hls::stream<custom_width> &pk_stream, hls::stream<custom_width> &e_2_stream)
{
	...
	#pragma HLS PIPELINE II=1
		...
		b ^= pk_tmp.range(671, 0) & e_tmp.range(671, 0);
...
void encryption_kernel(unsigned char *s_1, unsigned char *s_2, unsigned char *s_3, unsigned char *s_4, unsigned char *e, aligned_width *pk_1, aligned_width *pk_2, aligned_width *pk_3, aligned_width *pk_4, v_vector *v, key_vector *key)
{
	...
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=436 bundle=gmem4
    	#pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=576 bundle=gmem5
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=576 bundle=gmem6
	#pragma HLS INTERFACE m_axi     port=pk_3    offset=slave depth=576 bundle=gmem7
	#pragma HLS INTERFACE m_axi     port=pk_4    offset=slave depth=576 bundle=gmem8
