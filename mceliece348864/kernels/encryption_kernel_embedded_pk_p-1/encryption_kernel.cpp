...
static void gen_e(hls::stream<unsigned char> &e_1_stream_1, hls::stream<custom_width> &e_2_stream_1, hls::stream<unsigned char> &e_out_stream,  v_vector *v, key_vector *key)
{
	...
	#pragma HLS STREAM variable=e_1_internal_stream_1 depth=96
...
void axi_aligned_pk_reader(hls::stream<custom_width> &pk_aligned_stream){
...
//	}
	...
	int j;
	custom_width pk_stream_value;
	custom_width pk_value;
	...
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_VALUES)); i++){
		...
		pk_value = 0;
			...
			for(j=0; j<(ALIGNED_VALUES); j++){
				pk_value = *(pk+i*(ALIGNED_VALUES)+j);
				pk_stream_value |= pk_value<<(j*8);
			}
		pk_aligned_stream.write( pk_stream_value );
	}
...
//void encryption_kernel_active(unsigned char *s_1, unsigned char *e, aligned_width *pk_1, v_vector *v, key_vector *key)
void encryption_kernel(unsigned char *s_1, unsigned char *e, v_vector *v, key_vector *key)
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=96  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=436  bundle=gmem1
//    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=2040  bundle=gmem2
...
//	axi_aligned_pk_reader(pk_aligned_stream_1, pk_1);
	axi_aligned_pk_reader(pk_stream_1);
