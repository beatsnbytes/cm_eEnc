...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)); i++){
		#pragma HLS PIPELINE II=1
...
//TODO try with 64 bits and see how to keep the same hls optimizations
	...
	for (j = 0; j < 6; j++){
		...
		pk_tmp = pk_stream.read();
		e_tmp = e_2_stream.read();
		b ^= pk_tmp.range(383, 0) & e_tmp.range(383, 0);
...
void encryption_kernel(unsigned char *s_1, unsigned char *s_2, unsigned char *s_3, unsigned char *s_4, unsigned char *e, aligned_width *pk_1, aligned_width *pk_2, aligned_width *pk_3, aligned_width *pk_4, v_vector *v, key_vector *key)
{
    ...
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=2912 bundle=gmem
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=2912 bundle=gmem1
	#pragma HLS INTERFACE m_axi     port=pk_3    offset=slave depth=2912 bundle=gmem2
	#pragma HLS INTERFACE m_axi     port=pk_4    offset=slave depth=2912 bundle=gmem3
    #pragma HLS INTERFACE m_axi     port=v       offset=slave depth=1 bundle=gmem4
	#pragma HLS INTERFACE m_axi     port=key     offset=slave depth=1 bundle=gmem4
