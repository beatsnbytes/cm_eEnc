...
#define ALIGNED_VALUES 102
#define BIT_WIDTH 816
...
//    for(int i=0; i<((2*SYS_T)/8); i++){
	...
	#pragma HLS PIPELINE II=1
    	//Increment the initialization vector and put to stream
    	//TODO PROBLEM HERE FIX
    	//TODO put for functional correctness replace with good performance alternative
...
void dump_to_v_stream_update(hls::stream<v_vector> &v_stream_update, hls::stream<v_vector> &v_stream_internal){
	...
	int i;
    v_vector one_msb = ap_uint<128> ("0x01000000000000000000000000000000", 16);
    v_vector original_increment_msb = ap_uint<128> ("0x20000000000000000000000000000000", 16);

    v_vector current_v=0;
//    current_v = v_stream_internal.read() + original_increment_msb;
...
void randombytes(hls::stream<cipher_vector> &output_ciphertext_stream,  v_vector *v_local_vector_out, key_vector *key_local_vector_out, v_vector v_local_vector_in, key_vector key_local_vector_in)
{
	...
	#pragma HLS STREAM variable=v_stream_aes depth=32 //Contains 16 16B values that are incrementing
	...
	#pragma HLS STREAM variable=ciphertext_stream depth=32 //Contains 16 16B cipheretext values
...
//    xf::security::aes256EcbEncrypt(v_stream_aes, key_stream_aes, ciphertext_stream, ((SYS_T*2)/8));
    xf::security::aes256EcbEncrypt(v_stream_aes, key_stream_aes, ciphertext_stream, 16);
...
void dump_to_e_1_stream(hls::stream<unsigned char> &e_1_stream, hls::stream<unsigned char> &e_1_internal_stream){
	...
	for(i=0; i<(SYND_BYTES)/4; i++){
		e_1_stream.write(e_1_internal_stream.read());
	}
}

void dump_to_e_2_stream(hls::stream<custom_width> &e_2_stream_1, hls::stream<custom_width> &e_2_stream_2, hls::stream<custom_width> &e_2_stream_3, hls::stream<custom_width> &e_2_stream_4, unsigned char *e_2_mat){
	...
	int cnt;
	for(k=0; k<PK_NROWS/4; k++){
	#pragma HLS PIPELINE
		cnt=0;
		for(i=1; i<=(SYS_N/8 - SYND_BYTES); i++){
			e_2_tmp = e_2_mat[i-1];
...
void compute_indexes_mat(hls::stream<gf_value> &indexes_stream, v_vector *v, key_vector *key){
	...
	#pragma HLS STREAM variable=aes_ciphertext_stream depth=32
...
#pragma HLS ARRAY_PARTITION variable=indexes_mat factor=128
	...
	while (1)
//	for(int i=0; i<1; i++)
	//While loop that exits only upon successful error vector generation by the AES
	{
		approved=1;
		...
		compose_gf_values(indexes_mat, aes_ciphertext_stream);
...
void compute_e_streams(hls::stream<unsigned char> &e_1_internal_stream_1, hls::stream<unsigned char> &e_1_internal_stream_2, hls::stream<unsigned char> &e_1_internal_stream_3, hls::stream<unsigned char> &e_1_internal_stream_4, unsigned char *e_2_mat, hls::stream<unsigned char> &e_out_internal_stream, hls::stream<gf_value> &indexes_stream){
	...
	unsigned char e_int[SYS_N/8];
	int i, j;

	gf_value indexes_mat[SYS_T];
	#pragma HLS ARRAY_PARTITION variable=indexes_mat factor=96
	...
	#pragma HLS PIPELINE II=3
		...
		{
			//TODO fix
			indexes_mat_msb = indexes_mat[j].range(12, 3);
			...
			e_int[i] |= (1 << (indexes_mat_lsb)) & mask;
		}
		e_out_internal_stream.write(e_int[i]);
		if(i<SYND_BYTES/4){
			e_1_internal_stream_1.write(e_int[i]);
		}else if((i>=SYND_BYTES/4) && (i<2*SYND_BYTES/4)){
			e_1_internal_stream_2.write(e_int[i]);
		}else if((i>=2*SYND_BYTES/4) && (i<3*SYND_BYTES/4)){
			e_1_internal_stream_3.write(e_int[i]);
		}else if((i>=3*SYND_BYTES/4) && (i<SYND_BYTES)){
			e_1_internal_stream_4.write(e_int[i]);
		}else{
			e_2_mat[i - SYND_BYTES] = e_int[i];
		}
...
static void gen_e(hls::stream<unsigned char> &e_1_stream_1, hls::stream<unsigned char> &e_1_stream_2, hls::stream<unsigned char> &e_1_stream_3, hls::stream<unsigned char> &e_1_stream_4, hls::stream<custom_width> &e_2_stream_1, hls::stream<custom_width> &e_2_stream_2, hls::stream<custom_width> &e_2_stream_3, hls::stream<custom_width> &e_2_stream_4, hls::stream<unsigned char> &e_out_stream,  v_vector *v, key_vector *key)
{
	int i, j, eq, count;
...
#pragma HLS dataflow
	...
	compute_e_streams(e_1_internal_stream_1, e_1_internal_stream_2, e_1_internal_stream_3, e_1_internal_stream_4, e_2_mat, e_out_internal_stream, indexes_stream);
	...
	dump_to_e_2_stream(e_2_stream_1, e_2_stream_2, e_2_stream_3, e_2_stream_4, e_2_mat);
...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	int i;
	//TODO if it is not divided directly how much should I read?
	//In the case of 786*340=261120 --> 261120/128B=2040
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_BYTES*4)); i++){
		#pragma HLS PIPELINE II=1
...
void pk_stream_resizer(hls::stream<custom_width> &pk_stream, hls::stream<aligned_width> &pk_aligned_stream){
	...
	for(i=0; i<((PK_NROWS*PK_ROW_BYTES)/(ALIGNED_VALUES*4)); i++){
	#pragma HLS PIPELINE II=1
		if(remnant_bits<BIT_WIDTH){
			//read from the aligned stream 128 values
			aligned_value = pk_aligned_stream.read();

			curr_idx = BIT_WIDTH - remnant_bits;

			//write to the output stream one 105 bundleprint
//			bundle = aligned_value.range(curr_idx - 1, 0);
			...
			//save the rest to a complement variable
			remnant = aligned_value.range(ALIGNED_WIDTH-1, curr_idx);

			//update the remnant count
...
void encryption_kernel(unsigned char *s_1, unsigned char *s_2, unsigned char *s_3, unsigned char *s_4, unsigned char *e, aligned_width *pk_1, aligned_width *pk_2, aligned_width *pk_3, aligned_width *pk_4, v_vector *v, key_vector *key)
{
	#pragma HLS INTERFACE m_axi     port=s_1     offset=slave depth=52  bundle=gmem
	#pragma HLS INTERFACE m_axi     port=s_2     offset=slave depth=52  bundle=gmem1
	#pragma HLS INTERFACE m_axi     port=s_3     offset=slave depth=52  bundle=gmem2
	#pragma HLS INTERFACE m_axi     port=s_4     offset=slave depth=52  bundle=gmem3
	#pragma HLS INTERFACE m_axi     port=e       offset=slave depth=1024  bundle=gmem4
    #pragma HLS INTERFACE m_axi     port=pk_1    offset=slave depth=2652 bundle=gmem5
	#pragma HLS INTERFACE m_axi     port=pk_2    offset=slave depth=2652 bundle=gmem6
	#pragma HLS INTERFACE m_axi     port=pk_3    offset=slave depth=2652 bundle=gmem7
	#pragma HLS INTERFACE m_axi     port=pk_4    offset=slave depth=2652 bundle=gmem8
    ...
    #pragma HLS INTERFACE s_axilite port=s_3     bundle=control
	#pragma HLS INTERFACE s_axilite port=s_4     bundle=control	
	...
	static hls::stream<custom_width> pk_stream_3("pk stream 3");
	#pragma HLS STREAM variable=pk_stream_3 depth=2
	static hls::stream<custom_width> pk_stream_4("pk stream 4");
	...
	static hls::stream<unsigned char> e_1_stream_3("e_1 stream 3");
	#pragma HLS STREAM variable=e_1_stream_3 depth=2
	static hls::stream<unsigned char> e_1_stream_4("e_1 stream 4");
	...
	static hls::stream<custom_width> e_2_stream_3("e_2 stream 3");
	#pragma HLS STREAM variable=e_2_stream_3 depth=2
	static hls::stream<custom_width> e_2_stream_4("e_2 stream 4");
	...
	static hls::stream<b_width> b_stream_3("b stream 3");
	#pragma HLS STREAM variable=b_stream_3 depth=2
	static hls::stream<b_width> b_stream_4("b stream 4");
	#pragma HLS STREAM variable=b_stream_4 depth=2	
	...
	static hls::stream<aligned_width> pk_aligned_stream_3("pk aligned stream 3");
	#pragma HLS STREAM variable=pk_aligned_stream_3 depth=2
	static hls::stream<aligned_width> pk_aligned_stream_4("pk aligned stream 4");
	#pragma HLS STREAM variable=pk_aligned_stream_4 depth=2		
	...
	axi_aligned_pk_reader(pk_aligned_stream_3, pk_3);
	axi_aligned_pk_reader(pk_aligned_stream_4, pk_4);

	pk_stream_resizer(pk_stream_1, pk_aligned_stream_1);
	pk_stream_resizer(pk_stream_2, pk_aligned_stream_2);
	pk_stream_resizer(pk_stream_3, pk_aligned_stream_3);
	pk_stream_resizer(pk_stream_4, pk_aligned_stream_4);

	gen_e(e_1_stream_1, e_1_stream_2, e_1_stream_3, e_1_stream_4, e_2_stream_1, e_2_stream_2, e_2_stream_3, e_2_stream_4, e_out_stream, v, key);
	...
	compute_n_store_s(s_3, e_1_stream_3, b_stream_3);
	compute_n_store_s(s_4, e_1_stream_4, b_stream_4);
