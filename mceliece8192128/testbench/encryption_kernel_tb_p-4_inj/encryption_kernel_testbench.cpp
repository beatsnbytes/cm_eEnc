...
int main () { 
    ...
    //Inputs
    aligned_width *pk_reference_input_1 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)));
    aligned_width *pk_reference_input_2 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)));
    aligned_width *pk_reference_input_3 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)));
    aligned_width *pk_reference_input_4 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)));
    ...
    unsigned char *s_kernel_output_1 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/4);
    unsigned char *s_kernel_output_2 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/4);
    unsigned char *s_kernel_output_3 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/4);
    unsigned char *s_kernel_output_4 = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES)/4);
...
for(iter=3; iter<4; iter++){
	...
	if(iter==0){
		pk_fptr = fopen("./io_values/enc_1/pk_in_alt.dat","r");
		...
		pk_fptr = fopen("./io_values/enc_2/pk_in_alt.dat","r");
		...
		pk_fptr = fopen("./io_values/enc_3/pk_in_alt.dat","r");
		...
		pk_fptr = fopen("./io_values/enc_4/pk_in_alt.dat","r");
    ...
    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)); i++){
    	(pk_reference_input_3[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input_3[i]) |= pk[j]<<(j*8);
    	}
    }

    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+80))/(ALIGNED_BYTES*4)); i++){
    	(pk_reference_input_4[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input_4[i]) |= pk[j]<<(j*8);
    	}
    }
    ...
    *key_reference_input=0;
    ...
    encryption_kernel_active(s_kernel_output_1, s_kernel_output_2, s_kernel_output_3, s_kernel_output_4, e_kernel_output, pk_reference_input_1, pk_reference_input_2, pk_reference_input_3, pk_reference_input_4, v_reference_input, key_reference_input);
    ...
    for(i=0; i<(SYND_BYTES)/4; i++){
    	if(*(s_kernel_output_2 + i)!= *(s_golden_output + i + (SYND_BYTES)/4)){
    		printf("ERROR! Expected s_2[%d]=%d but got %d\n", i, *(s_golden_output + i + (SYND_BYTES)/4), *(s_kernel_output_2 + i));
    		ret=1;
    	}
    }
    ...
    for(i=0; i<(SYND_BYTES)/4; i++){
    	if(*(s_kernel_output_3 + i)!= *(s_golden_output + i + 2*(SYND_BYTES)/4)){
    		printf("ERROR! Expected s_3[%d]=%d but got %d\n", i, *(s_golden_output + i + 2*(SYND_BYTES)/4), *(s_kernel_output_3 + i));
    		ret=1;
    	}
    }
    ...
    for(i=0; i<(SYND_BYTES)/4; i++){
    	if(*(s_kernel_output_4 + i)!= *(s_golden_output + i + 3*(SYND_BYTES)/4)){
    		printf("ERROR! Expected s_4[%d]=%d but got %d\n", i, *(s_golden_output + i + (SYND_BYTES)/4), *(s_kernel_output_4 + i));
    		ret=1;
    	}
    }
