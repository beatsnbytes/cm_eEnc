#ifndef PARAMS_H
#define PARAMS_H

#define GFBITS 13
#define SYS_N 8192
#define SYS_T 128

#define COND_BYTES ((1 << (GFBITS-4))*(2*GFBITS - 1))
#define IRR_BYTES (SYS_T * 2)

#define PK_NROWS (SYS_T*GFBITS) 
#define PK_NCOLS (SYS_N - PK_NROWS)
#define PK_ROW_BYTES ((PK_NCOLS + 7)/8)

#define SYND_BYTES ((PK_NROWS + 7)/8)

#define GFMASK ((1 << GFBITS) - 1)

//TODO Define through command line arguments?
#define PARALLELIZATION_FACTOR 1

#define ENCRYPTION_KERNEL 
//#undef ENCRYPTION_KERNEL 

//#define DECRYPTION_KERNEL 
#undef DECRYPTION_KERNEL 

//#define ALVEO
#undef ALVEO

#define ZCU
//#undef ZCU



#endif

