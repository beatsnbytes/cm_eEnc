...
void dump_to_e_2_stream(hls::stream<custom_width> &e_2_stream, unsigned char *e_2_mat){
	...
	#pragma HLS PIPELINE
		...
		for(i=1; i<=(677); i++){
			...
			if(((i)%(PACK_FACTOR))==0){
				e_2_stream.write(e_2_stream_value);
				e_2_stream_value = 0;
			}
...
//	#pragma HLS STREAM variable=gf_values_stream depth=476 //There should be in total 128 values. In case of dataflow this wont be full probably.
...
void compute_e_streams(hls::stream<unsigned char> &e_1_internal_stream, hls::stream<unsigned char> &e_2_internal_stream, hls::stream<unsigned char> &e_out_internal_stream, hls::stream<gf_value> &indexes_stream){
...
static void gen_e(hls::stream<unsigned char> &e_1_stream, hls::stream<custom_width> &e_2_stream, hls::stream<unsigned char> &e_out_stream,  v_vector *v, key_vector *key)
{
	int i, j, eq, count;
    ...
    static hls::stream<unsigned char> e_1_internal_stream("e_1_internal stream");
	#pragma HLS STREAM variable=e_1_internal_stream depth=194
...
#pragma HLS dataflow
	...
	compute_e_streams(e_1_internal_stream, e_2_internal_stream, e_out_internal_stream, indexes_stream);

	dump_to_e_1_stream(e_1_stream, e_1_internal_stream);

	e_tail_handling(e_2_mat, e_2_internal_stream);
	dump_to_e_2_stream(e_2_stream, e_2_mat);
...
void axi_aligned_pk_reader(hls::stream<aligned_width> &pk_aligned_stream, aligned_width *pk){
	...
	int i;
	LOOP_LOAD_PK:
	for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES)); i++){
		#pragma HLS PIPELINE II=1
...
void encryption_kernel(unsigned char *s, unsigned char *e, aligned_width *pk, v_vector *v, key_vector *key)
{
    ...
    #pragma HLS INTERFACE m_axi     port=pk     offset=slave depth=9282 bundle=gmem2
	...
	static hls::stream<custom_width> pk_stream("pk stream");
	#pragma HLS STREAM variable=pk_stream depth=2//4641

	static hls::stream<unsigned char> e_1_stream("e_1 stream");
	#pragma HLS STREAM variable=e_1_stream depth=10//194

	static hls::stream<custom_width> e_2_stream("e_2 stream");
	#pragma HLS STREAM variable=e_2_stream depth=2//4641 //9282

	static hls::stream<unsigned char> e_out_stream("e_out stream");
	#pragma HLS STREAM variable=e_out_stream depth=2//870

	static hls::stream<b_width> b_stream("b stream");
	#pragma HLS STREAM variable=b_stream depth=2

	#pragma HLS dataflow

	axi_aligned_pk_reader(pk_stream, pk);

	gen_e(e_1_stream, e_2_stream, e_out_stream, v, key);

	compute_b_mat(b_stream, pk_stream, e_2_stream);

	compute_n_store_s(s, e_1_stream, b_stream);
