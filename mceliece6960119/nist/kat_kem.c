    ...
    #ifdef ENCRYPTION_KERNEL
	{
		...
		#ifdef ALVEO
        {
    		...
    		for(i=0; i<PARALLELIZATION_FACTOR; i++){
    			posix_memalign((unsigned char **)&ptr_pk_in[i], 4096, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+91))/PARALLELIZATION_FACTOR);
    	    ...
    	    for(i=0; i<PARALLELIZATION_FACTOR; i++){
    	        //PK
    	        buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+91))/PARALLELIZATION_FACTOR, ptr_pk_in[i], NULL);
    	        //S
		...
		#ifdef ZCU
        {
			...
			for(i=0; i<PARALLELIZATION_FACTOR; i++){
				//PK
				buffer_pk_in[i] = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+91))/PARALLELIZATION_FACTOR, NULL, &err);
				//S
			...
			for(i=0; i<PARALLELIZATION_FACTOR; i++){
				//PK
				ptr_pk_in[i] = (unsigned char *) clEnqueueMapBuffer(commands, buffer_pk_in[i], true, CL_MAP_WRITE, 0, sizeof(unsigned char)*(PK_NROWS*(PK_ROW_BYTES+91))/PARALLELIZATION_FACTOR, 0, NULL, NULL, &err);
				//S_BUFF
