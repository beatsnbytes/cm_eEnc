...
int main () { 
    ...
    //Inputs
    aligned_width *pk_reference_input_1 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_2 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_3 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_4 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_5 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_6 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    aligned_width *pk_reference_input_7 = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)));
    ...
    unsigned char *s_kernel_output = (unsigned char *) malloc(sizeof(unsigned char)*(SYND_BYTES));
...
for(iter=0; iter<4; iter++){
    ...
    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)); i++){
    	(pk_reference_input_5[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input_5[i]) |= pk[j]<<(j*8);
    	}
    }

    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)); i++){
    	(pk_reference_input_6[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input_6[i]) |= pk[j]<<(j*8);
    	}
    }

    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES*7)); i++){
    	(pk_reference_input_7[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input_7[i]) |= pk[j]<<(j*8);
    	}
    }
...
//		printf("key = %d\n", key.to_uint());
    ...
    encryption_kernel_active(s_kernel_output, e_kernel_output, pk_reference_input_1, pk_reference_input_2, pk_reference_input_3, pk_reference_input_4, pk_reference_input_5, pk_reference_input_6, pk_reference_input_7, v_reference_input, key_reference_input);

    
    // Compare the results of the function against expected results
    
    for(i=0; i<(SYND_BYTES); i++){
    	if(*(s_kernel_output + i)!=*(s_golden_output + i)){
    		printf("ERROR! Expected s_1[%d]=%d but got %d\n", i, *(s_golden_output + i), *(s_kernel_output + i));
    		ret=1;
    	}
    }
