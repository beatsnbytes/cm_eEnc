...
int main () { 
    ...
    //Inputs
    aligned_width *pk_reference_input = (aligned_width *) malloc(sizeof(aligned_width)*((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES)));
...
for(iter=0; iter<4; iter++){
    ...
    for(i=0; i<((PK_NROWS*(PK_ROW_BYTES+91))/(ALIGNED_BYTES)); i++){
    	(pk_reference_input[i]) = 0;
    	for(j=0; j<(ALIGNED_BYTES); j++){
    		pk[j] = 0;
    		fscanf(pk_fptr, "%d\n", (pk + j));
    		(pk_reference_input[i]) |= pk[j]<<(j*8);
    	}
    }
///////////////////////////////END PK//////////////////////////////////////////
...
//		printf("key = %d\n", key.to_uint());
    ...
    encryption_kernel_active(s_kernel_output, e_kernel_output, pk_reference_input, v_reference_input, key_reference_input);
